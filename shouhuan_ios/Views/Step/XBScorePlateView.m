...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.speedValues=6.0;
        self.backgroundColor = [UIColor clearColor];
        self.startAngle=-0.35*M_PI;
        self.endAngle = 1.35*M_PI;
        self.comments =@"目标步数3000";
        self.backgroundColor = [UIColor clearColor];
        _cur_compV=0;
        _cur_speedV=0;
        _cur_altitudeV=0;
...
//    self.lbeCompreValue.text =@"2018/01/05";
...
-(void)drawHalfCircle:(CGPoint) center andWithRadius:(CGFloat)radius isTop:(BOOL) top
{
    ...
    NSLog(@"borderValue----%f",borderValue);
    //实现动画效果，只能先画刻度，再画具体值
    for (int i=1; i<self.precision; i++)//画刻度
    {
        
        CGContextSetRGBStrokeColor(ctx, 145.0/255, 151.0/255, 162.0/255, 1);//设置白色 透明
    ...
    for (int i=1; i<self.precision; i++)
    {
        ...
        NSLog(@"curAngle----%f",curAngle);
        
        if ((double)i/(double)self.precision<borderValue)
        {
            CGContextSetRGBStrokeColor(ctx, 59.0/255, 199.0/255, 95.0/255, 1);//设置白色 不透明
            CGFloat endX,endY,startX,startY;//刻度的长度是这里 7 -2 =5；
            if (top) {
